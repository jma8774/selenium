              ...
              try:
                while offset < total:
                  ...
                  if (response.status_code != 200):
                    log.error(f"❌ Target returned status code {response.status_code} for {full_url}")
                    log.error(response)
                    continue
                  json = response.json()
                  total = try_get_json_value(json, "data.search.search_response.metadata.total_results") or 9999999
                  products = try_get_json_value(json, "data.search.products") or []
                  for product in products:
                      title = try_get_json_value(product, "item.product_description.title")
                      buy_url = try_get_json_value(product, "item.enrichment.buy_url")
                      tcin = try_get_json_value(product, "tcin")
                      if tcin not in db_tcins:
                          log.info(f"🆕 New product: {title} ({tcin}): {buy_url}")
                          sendBotChannel(f"🆕 New product: {title} ({tcin}): {buy_url}")
                          db_tcins.add(tcin)
                  offset += 24
                  time.sleep(0.25)
...
def check_fn(response: requests.Response) -> bool:
  json = response.json()
...
def on_in_stock(poller: Poller, response):
    json = response.json()
