...
class MyBot(discord.Client):
  ...
  async def send(self, channel, message, user:str=None, role:str=None, delete_after=None):
    global channels, users, roles
    # log.info(f"Sending message to {channel}: {message}")
    if user:
      await channels[channel].send(f"<@{users[user]}> " + message, delete_after=delete_after)
    elif role:
      await channels[channel].send(f"<@&{roles[role]}> " + message, delete_after=delete_after)
    else:
...
def send(channel, message, user:str=None, role:str=None, delete_after=None):
  ...
  if not discord_client or not discord_loop or args.dev:
    return
  asyncio.run_coroutine_threadsafe(
    discord_client.send(channel, message, user, role, delete_after),
    discord_loop
  )
